...
def tokenizza_proposizione_lis(proposizione_testo, attore_scelto):
    ...
    for token in doc:
        ...
        # --- GESTIONE NOMI PROPRI (DATTILOLOGIA AUTOMATICA) ---
        if pos == "PROPN":
            # Scomponiamo il nome in una lista di lettere minuscole: ['m', 'a', 'r', 'c', 'o']
            sequenza_lettere = [lettera.lower() for lettera in parola if lettera.isalpha()]

            print(
                f"  [DATTILOLOGIA] Attivato spelling automatico per il nome proprio: '{parola}' -> {sequenza_lettere}")
            ...
            continue  # Passa subito alla parola successiva senza cercare nel DB!
